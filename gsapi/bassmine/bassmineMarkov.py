...
def constrainMM(markov_model, target, _path="output/"):
    ...
    target = bassmineUtils.translate_rhythm(
        bassmineUtils.binaryBeatPattern([e.startTime for e in target.events], target.duration))
...
def generateBassRhythm(markov_model, beat_length=8, target=[]):
    ...
    for idx in pattern_idx:
        ...
        if len(st) > 0:
            for s in st:
                bassline.events.append(Event(s + beat_count, 0.25, 36, velocity=110, tag="bass"))
        beat_count += 1
...
def generateBassRhythmVariation(markov_model, target_pattern, variation_mask):
    ...
    if len(variation_mask) < target_pattern.duration:
        ...
        target_rhythm = bassmineUtils.binaryBeatPattern(onset_target, target_pattern.duration)
        target_id = bassmineUtils.translate_rhythm(target_rhythm)
...
def corpus_analysis(bass_path, drum_path):
    ...
    for f in range(len(drum_files)):
        ...
        # Read Bassline files
        bass_pattern = io.fromMidi(bass_files[f], "pitchNames", TagsFromTrackNameEvents=False)
        ...
        # Read Drum files
        # Filter kick notes
        kick_pattern = io.fromMidi(match_file, {"Kick": 36}, TagsFromTrackNameEvents=False)
