...
gsmarkovLog = logging.getLogger("gsapi.gsmarkov")
...
class PatternMarkov(object):
    ...
    def generateTransitionTableFromPatternList(self, patternClasses):
        ...
        if not isinstance(patternClasses, list):
            gsmarkovLog.error("PatternMarkov need a list")
            return False
        else:
    ...
    def generatePattern(self, seed=None):
            ...
            if previousTags not in self.transitionTable[step]:
                gsmarkovLog.error(
                    "wrong transition table, zero state for " + str(
                        previousTags))
                gsmarkovLog.error(str(self.transitionTable[step]))
                return None
        ...
        for el in events:
            for tagElem in el:
                if tagElem != 'silence':
                    pattern.events += [
                        gspattern.Event(idx * stepSize, stepSize, 100, 127, tag=tagElem)]
    ...
    def checkSilences(self, p):
        for i in range(int(p.duration)):
            ...
            if len(c) > 1 and ("silence" in c):
                gsmarkovLog.error("wrong silence")
    ...
    def plotGlobalMatrix(self):
        ...
        possibleStatesIn = list(
                set([item for step in range(numSteps) for item in
