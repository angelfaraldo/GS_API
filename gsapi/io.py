...
iolog = logging.getLogger("gsapi.io")
...
def __fromMidiFormatted(midiPath,
    ...
    extremeLog = iolog.getEffectiveLevel() <= logging.DEBUG
        ...
        for e in tracks:
            ...
            if isNoteOn or isNoteOff:
                ...
                if isNoteOn:
                    ...
                    pattern.events += [Event(startTime=curBeat,
                                             duration=-1,
                                             pitch=pitch,
                                             velocity=velocity,
                                             tag=noteTag)]
                ...
                if isNoteOff:
                    if extremeLog:
                        iolog.debug("off %d %f" % (pitch, curBeat))
                    foundNoteOn = False
