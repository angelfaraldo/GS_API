...
listOfStyle = map(os.path.basename, glob.glob(os.path.join(
    VSTPlugin.localDirectory, "midi", "*.mid")))
listOfStyle+=["*"]
...
def configureParams():
	VSTPlugin.loopDuration.setMinMax(1, 16).setCallbackFunction(
        VSTPlugin.generateStyleIfNeeded, forceParamUpdate=True)
	VSTPlugin.numSteps.setMinMax(4, 32).setCallbackFunction(
        VSTPlugin.generateStyleIfNeeded, forceParamUpdate=True)
...
def updateTog(param):
	if param.value != None:
		...
		print "eventTrig"
	JUCEAPI.vst.setPattern(
        VSTPlugin.mapMidi(VSTPlugin.style.generatePattern()))
