...
patternLog = python.gsapi.logger.getLogger("gsapi.pattern.pattern")


class Pattern(object):
    """Class representing a pattern made of Event.
    Holds a list of events and provide basic manipulation function.
    ...
    Class Variables:
        ...
        events: list of Event for this pattern.
    ...
    def generateViewpoint(self, name, descriptor=None, sliceType=None):
        ...
        def _computeViewpoint(originPattern, descriptor, name, sliceType=1):
            ...
            for subPattern in patternsList:
                if subPattern:
                    viewpoint.events+=[Event(duration=subPattern.duration,startTime=subPattern.startTime,tag = descriptor.getDescriptorForPattern(subPattern),originPattern=subPattern)]
        ...
        if descriptor :
            ...
            if name == "chords":
                from .descriptors.DescriptorChord import DescriptorChord
                self.viewpoints[name] = _computeViewpoint(originPattern=self, descriptor=DescriptorChord(), sliceType=4, name=name)
