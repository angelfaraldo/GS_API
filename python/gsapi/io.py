...
iolog = python.gsapi.utils.Logging.getLogger("gsapi.io")
# iolog.setLevel(level=logging.INFO)
...
def fromJSONFile(filePath, conserveTuple=False):
    ...
    with open(filePath, 'r') as f:
        return pattern().fromJSONDict(json.load(f, object_hook=hinted_tuple_hook if conserveTuple else None))
...
def toJSONFile(pattern, folderPath, useTagIndexing=True, nameSuffix=None, conserveTuple = False):
    ...
    with open(filePath, 'w') as f:
        json.dump(pattern.toJSONDict(useTagIndexing=useTagIndexing),
                  f,
                  cls=encoderClass,
                  indent=1,
                  separators=(',', ':'))
...
def __fromMidiFormatted(midiPath,
    ...
    extremeLog = iolog.getEffectiveLevel() <= python.gsapi.utils.Logging.DEBUG

    # get time signature first
    iolog.info("start processing %s" % pattern.name)
            ...
            if midi.MetaEvent.is_event(e.statusmsg):
                if e.metacommand == midi.TrackNameEvent.metacommand:
                    if tracksToGet and not((e.text in tracksToGet) or (trackIdx in tracksToGet)):
                        iolog.info("skipping track: %i %s" % (trackIdx, e.text))
                        shouldSkipTrack = True
                        continue
                    else:
                        iolog.info(pattern.name + ": getting track: %i %s" % (trackIdx, e.text))
            ...
            if isNoteOn or isNoteOff:
                ...
                if not noteTag:
                    if [e.channel, pitch] not in notFoundTags:
                        iolog.info(pattern.name + ": no tags found for "
                ...
                if isNoteOn:
                    if extremeLog : iolog.debug("on %d %f" % (pitch, curBeat))
                    pattern.events += [PatternEvent(startTime=curBeat,
                                                      duration=-1,
                                                      pitch=pitch,
                                                      velocity=velocity,
                                                      tag=noteTag)]
                ...
                if isNoteOff:
                    if extremeLog: iolog.debug("off %d %f" % (pitch, curBeat))
                    foundNoteOn = False
                    ...
                    for i in reversed(pattern.events):
                        ...
                        if (i.pitch == pitch) and (i.tag==noteTag) and ((isTrueNoteOff and(curBeat >= i.startTime))or curBeat>i.startTime) and i.duration<=0.0001:
                            ...
                            iolog.info("set duration %f at start %f " % (i.duration, i.startTime))
                            break
                    if not foundNoteOn :

                        iolog.warning(pattern.name + ": not found note on\n %s\n%s\n%s , %s " % (e, pattern.events[-1], noteTag, curBeat))
...
def __findTimeInfoFromMidi(pattern, midiFile):
            ...
            if midi.MetaEvent.is_event(e.statusmsg):
                if e.metacommand == midi.TimeSignatureEvent.metacommand:
                    if foundTimeSignatureEvent and (pattern.timeSignature != (e.numerator, e.denominator)):
                        iolog.error(pattern.name + ": multiple time "
                    ...
                    if foundTempo:
                        iolog.error(pattern.name + ": multiple bpm found, not supported")
                    foundTempo = True
                    pattern.bpm = e.bpm
    ...
    if not foundTimeSignatureEvent:
        iolog.info(pattern.name + ": no time signature event found")
